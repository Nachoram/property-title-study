-- Migration 029: Complete Phased Submission Infrastructure
-- 1. Ensure ALL ocr_ tables have the 'fase' column
DO $$ 
DECLARE 
    t text;
BEGIN 
...
-- 2. Update the RPC function to be DYNAMIC (no hardcoded table list) and de-duplicate
...
BEGIN
  ...
  FOR v_t IN 
    ...
    WHERE table_schema = 'public' 
      AND table_name LIKE 'ocr_%'
      -- Avoid sub-tables or join tables that don't have operacion_id/documento_url directly if needed
      -- For now, we assume all OCR entry tables have these columns if they are meant to be gathered
  LOOP
