-- Migration: Rename ocr_inscripcion_herencia to ocr_inscripcion_posesion_efectiva
-- Date: 2026-02-18
-- Description: Renames the table and updates the document name to reflect the correct terminology.

-- 1. Rename the table
ALTER TABLE IF EXISTS public.ocr_inscripcion_herencia RENAME TO ocr_inscripcion_posesion_efectiva;

-- 2. Update the default value for nombre_documento_ocr
ALTER TABLE public.ocr_inscripcion_posesion_efectiva 
  ALTER COLUMN nombre_documento_ocr SET DEFAULT 'Inscripción de Posesión Efectiva';

-- 3. Update existing records
UPDATE public.ocr_inscripcion_posesion_efectiva 
SET nombre_documento_ocr = 'Inscripción de Posesión Efectiva'
WHERE nombre_documento_ocr = 'Inscripción Especial de Herencia';

-- 4. Update solicitud_documentos to maintain consistency
UPDATE public.solicitud_documentos
SET tipo_documento = 'inscripcion_posesion_efectiva',
    nombre_documento = 'Inscripción de Posesión Efectiva'
WHERE tipo_documento = 'inscripcion_herencia';

-- 5. Update the handle_storage_upload trigger function
...
BEGIN
  ...
  ELSIF v_filename ILIKE '%posesion%' THEN
    -- Check if it is an inscription or the base resolution
    IF v_filename ILIKE '%inscripcion%' THEN
      v_table_name := 'ocr_inscripcion_posesion_efectiva';
    ELSE
  ...
  ELSIF v_filename ILIKE '%herencia%' THEN
    ...
    ELSE
      -- Legacy support for filenames with 'herencia'
      v_table_name := 'ocr_inscripcion_posesion_efectiva';
    END IF;
