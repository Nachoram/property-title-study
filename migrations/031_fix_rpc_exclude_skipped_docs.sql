-- Migration 031: Fix RPC to exclude 'No Aplica' documents from solicitud_documentos
-- This prevents skipped documents (which might retain a URL) from being sent to review.
...
BEGIN
  ...
  -- FIX: Exclude documents where estado is 'No Aplica'
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'solicitud_documentos') THEN
      v_query := format('SELECT jsonb_agg(jsonb_build_object(''table'', ''solicitud_documentos'', ''id'', id, ''url'', documento_url, ''fase'', fase)) FROM solicitud_documentos WHERE operacion_id = %L AND documento_url IS NOT NULL AND estado != ''No Aplica''', p_numero_operacion);
